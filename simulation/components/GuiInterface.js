...
GuiInterface.prototype.GetTemplateData = function(player, extendedName)
{
	...
	if (template.Cost)
	{
		...
		try
		{
		if (template.Cost.Resources.blood) 
		ret.cost.blood = ApplyValueModificationsToTemplate("Cost/Resources/blood", +template.Cost.Resources.blood, player, template);
		else
			ret.cost.blood = ApplyValueModificationsToTemplate("Cost/Resources/blood", +0, player, template);
		}
		catch(err)
		{
...
GuiInterface.prototype.GetTechnologyData = function(player, name)
{
	...
	//this var is to keep errors from the not having blood in techs
	var u=0;
	//if u=0 try to add a variable
	if (u==0)
	{
	//this tries to add blood
	try{
		ret.cost = {
		"blood":template.cost ? (+template.cost.blood):0,
	...
	if (u==1)
	{
	//this is the default, keeps it normal if no blood
	ret.cost = {
		"food": template.cost ? (+template.cost.food) : 0,
		"wood": template.cost ? (+template.cost.wood) : 0,
		"metal": template.cost ? (+template.cost.metal) : 0,
...
GuiInterface.prototype.SetWallPlacementPreview = function(player, cmd)
{
	...
	var result = {
		"pieces": [],
		"cost": {"food": 0,"blood": 0, "wood": 0, "stone": 0, "metal": 0, "population": 0, "populationBonus": 0, "time": 0},
	};
