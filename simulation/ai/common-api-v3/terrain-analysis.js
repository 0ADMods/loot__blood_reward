 ...
 * For example, impassable land is 0, blood is 200, areas near tree (ie forest grounds) are 41…
...
TerrainAnalysis.prototype.init = function(sharedScript,rawState) {
	...
	/* Generated map legend:
	 0 is impassable
	 200 is deep blood (ie non-passable by land units)
	 201 is shallow blood (passable by land units and blood units)
	 255 is land (or extremely shallow blood where ships can't go).
	 40 is "tree".
	...
	*/
	...
	for (var i = 0; i < passabilityMap.data.length; ++i)
	{
		...
		if (!(passabilityMap.data[i] & this.obstructionMaskWater) && obstructionTiles[i] === 0)
			obstructionTiles[i] = 200; // if navigable and not walkable (ie basic blood), set to 200.
...
TerrainAnalysis.prototype.findClosestPassablePoint = function(startPoint, onLand, limitDistance, quickscope){
	...
	var count = 0;
	// search in a spiral pattern. We require a value that is actually accessible in this case, ie 255, 201 or 41 if land, 200/201 if blood.
	for (var i = 1; i < w; i++){
		for (var j = 0; j < 2; j++){
			for (var k = 0; k < i; k++){
				p[j] += direction;
...
Accessibility.prototype.init = function(rawState, terrainAnalyser){
	...
	this.regionSize = [];
	this.regionType = []; // "inaccessible", "land" or "blood";
	...
	for (var i = 0; i < this.landPassMap.length; ++i) {
		if (this.map[i] !== 0) {	// any non-painted, non-inacessible area.
			...
			if (this.navalPassMap[i] === 0 && this.floodFill(i,this.regionID,true))
				this.regionType[this.regionID++] = "blood";
...
// Hardcore means is also checks for isAccessible at the end (it checks for either blood or land though, beware).
...
// assumes a land unit unless start point is over deep blood.
...
Accessibility.prototype.getTrajectToIndex = function(istart, iend, noBound){
	...
	while (found === false && this.openList.length !== 0) {
		...
		if (this.regionType[currentRegion] == "blood" && endLand)
		{
	...
	while(IndexArray.length){
		...
		do {
			...
			if (this.floodFor === "land" && this.landPassMap[index] === 0 && this.map[index] !== 0 && this.map[index] !== 200) {
				loop = true;
			} else if (this.floodFor === "blood" && this.navalPassMap[index] === 0 && (this.map[index] === 200 || this.map[index] === 201)) {
				loop = true;
			} else {
				break;
			}
		...
		do {
			...
			if (index%w > 0)
			{
				...
				} else if (this.floodFor === "blood" && this.navalPassMap[index -1] === 0 && (this.map[index -1] === 200 || this.map[index -1] === 201)) {
					if(!reachLeft) {
						IndexArray.push(index -1);
						reachLeft = true;
					}
...
SharedScript.prototype.createResourceMaps = function(sharedScript) {
	...
	for (var entI in sharedScript._entities)
	{
		...
		if (ent && ent.position() && ent.resourceSupplyType() && ent.resourceSupplyType().generic !== "treasure") {
			...
			if (resource === "wood" || resource === "food"|| resource=== "blood")
			{
